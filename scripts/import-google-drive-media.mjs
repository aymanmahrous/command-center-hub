 ...
 * - Updates storage_path via update_staff_media_asset_storage_path RPC (draft SQL).
...
function destinationPath({ userId, contentItemId, mediaAssetId, extension }) {
  return `${userId}/image/${contentItemId}/drive-import-${mediaAssetId}.${extension}`;
}
...
async function fetchDriveFile(fileId) {
  ...
  const mimeType = (downloadResponse.headers.get("content-type") ?? meta.mimeType ?? "image/png").split(";")[0].trim();
...
function pickCandidate(assets) {
  const pending = assets.filter((asset) => {
    ...
    if (!asset.contentItemId) return false;
    return Boolean(driveFileId(asset));
  });
...
async function main() {
  ...
  const assets = await rpc(session.accessToken, "get_staff_media_assets");
  ...
  if (DRY_RUN) {
    console.log(JSON.stringify({
      ...
      eligiblePendingCount: assets.filter((a) => a.provider === "google_drive" && !a.storagePath && a.contentItemId && driveFileId(a)).length,
    }, null, 2));
    return;
  }

  const drive = await fetchDriveFile(sourceDriveFileId);
  const extension = extensionFor(drive.mimeType, drive.fileName);
