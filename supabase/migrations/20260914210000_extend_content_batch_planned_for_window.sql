-- Extend planned_for horizon so 30-day Coach Ayman calendars can be created safely.
-- Additive: preserves media linkage, audit logging, and validation behavior.
...
begin
  ...
  loop
    ...
    if v_planned_for <= now() or v_planned_for > now() + interval '45 days' then
      raise exception 'INVALID_PLANNED_TIME' using errcode = '22023';
    end if;
