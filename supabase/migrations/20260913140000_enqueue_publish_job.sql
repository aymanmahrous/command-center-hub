-- Phase 2: staff-facing publish enqueue for Command Center Hub.
-- Creates owner_publish_authorizations + publish_content background job atomically.
-- Does not call n8n, Buffer, or Meta directly.
...
begin
  ...
  if v_content.published_at is not null or nullif(btrim(coalesce(v_content.provider_external_id, '')), '') is not null then
  ...
  insert into public.background_jobs (
    ...
    jsonb_build_object(
      ...
      'source', 'command_center_request_publish',
...
REVOKE ALL ON FUNCTION public.enqueue_publish_job(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enqueue_publish_job(uuid) TO authenticated, service_role;
