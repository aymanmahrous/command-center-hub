-- Minimal fix: prioritize CURRENT inbound message over stale concierge intent.
-- Draft-only. No outbound send. No schema changes.
...
begin
  ...
  if v_body ~* '(human|person|coach|ayman|speak to someone|talk to someone|موظف|بشري|كوتش|أيمن|مدرب|تحدث مع)' then
    ...
    v_draft := case
      when v_language = 'ar'
        then 'سأوصلك بالكوتش أيمن لتفاصيل موقع التدريب الدقيقة.'
      else 'I''ll connect you with Coach Ayman for the exact training location details.'
    end;
    v_stage := case when v_stage = 'new' then 'contacted' else v_stage end;
    v_score := v_score + 10;
    v_processed := true;
    ...
    v_processed := true;
        ...
        or (
          (
            ...
            or v_body ~* '^(نعم|أيوه|ايوه|تمام|موافق)($|[[:space:]])'
          )
          and v_prior_state in ('presented_pricing', 'booking_guidance')
        ) then
    ...
    v_processed := true;
        ...
        or v_body ~* '^(السلام عليكم|عليكم السلام|سلام|مرحبا|مرحبًا|صباح الخير|مساء الخير|أهلا|اهلا|أهلان)($|[[:space:]])'
    ...
    v_draft := case
      when v_language = 'ar'
        then 'سأوصلك بالكوتش أيمن ليجيبك بدقة على هذا السؤال.'
      else 'I''ll connect you with Coach Ayman so he can answer this accurately.'
    end;
    v_stage := case when v_stage = 'new' then 'contacted' else v_stage end;
    v_score := v_score + 10;
    v_processed := true;
