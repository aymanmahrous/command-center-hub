-- Final Instagram account-ID gate correction for verified @relaxfixuae
-- Replace explicit Instagram account validation ID:
--   previous configured: 17841400516601494
--   Graph-verified:      17841400516801494
-- Scope: only listed Instagram RPCs that hardcode/validate the account ID.
-- No publishing logic, Facebook/WhatsApp, or historical row changes.


CREATE OR REPLACE FUNCTION public.authorize_and_enqueue_instagram_publish(p_content_item_id uuid, p_platform text, p_account_id text, p_authorized_by text, p_approval_reference text, p_ttl_minutes integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if p_account_id <> '17841400516801494' then
...
CREATE OR REPLACE FUNCTION public.claim_authorized_instagram_publish_job(p_content_item_id uuid, p_platform text, p_account_id text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
begin
  ...
  if p_platform <> 'instagram' or p_account_id <> '17841400516801494' then
...
CREATE OR REPLACE FUNCTION public.confirm_failed_instagram_publish_not_sent(p_authorization_id uuid, p_job_id uuid, p_content_item_id uuid, p_evidence_reference text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
begin
  ...
  if not found
     ...
     or v_auth.page_id <> '17841400516801494' then
...
CREATE OR REPLACE FUNCTION public.create_owner_instagram_publish_authorization(p_content_item_id uuid, p_platform text, p_account_id text, p_authorized_by text, p_approval_reference text, p_ttl_minutes integer DEFAULT 15)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
begin
  ...
  inner join public.owner_publish_authorizations opa
    ...
    and opa.page_id = '17841400516801494'
...
CREATE OR REPLACE FUNCTION public.fail_authorized_instagram_publish_job(p_job_id uuid, p_authorization_id uuid, p_content_item_id uuid, p_platform text, p_account_id text, p_outcome text, p_error text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.mark_authorized_instagram_publication_published(p_authorization_id uuid, p_job_id uuid, p_content_item_id uuid, p_platform text, p_account_id text, p_external_post_id text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.reject_authorized_instagram_publish_job(p_job_id uuid, p_authorization_id uuid, p_content_item_id uuid, p_platform text, p_account_id text, p_reason text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.reserve_authorized_instagram_publication_receipt(p_authorization_id uuid, p_job_id uuid, p_content_item_id uuid, p_platform text, p_account_id text, p_provider text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
