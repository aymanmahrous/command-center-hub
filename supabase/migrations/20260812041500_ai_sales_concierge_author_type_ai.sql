-- Fix process_ai_sales_concierge_turn draft insert author_type.
-- Root cause: drafts used 'ai_draft' which violates messages_author_type_check
-- (allowed: customer | ai | human | system). Prior fix was proposed but NEVER
-- applied as a migration; phone-ID migrations only replaced
-- process_whatsapp_webhook_ingress and did not revert this function.
-- Recreate latest clarifier body with author_type = 'ai'.
...
begin
  ...
  if v_processed and v_draft is null then
    if v_state = 'awaiting_offer_type' then
      ...
      -- Sticky-state fix: if already in presented_pricing and message is not pricing/booking,
      -- ask a short clarifier instead of reprinting the full offer block.
      if v_prior_state = 'presented_pricing'
         and v_body !~* '(price|cost|how much|fee|aed|dirham|سعر|كم|تكلفة|درهم)'
         and v_body !~* '(book|booking|reserve|appointment|subscribe|حجز|موعد|اشتراك|أحجز|احجز)' then
        v_draft := case
          when v_language = 'ar'
            then 'هل تفضّل حصة خاصة أم مجموعة، وهل نتابع للحجز؟'
          else 'Would you like a private lesson or a group lesson, and shall we proceed with booking?'
        end;
      else
        v_draft := case
