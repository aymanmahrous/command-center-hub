-- Phase 6C: ingest WhatsApp image/audio/voice metadata into existing CRM (no media download).
...
declare
  ...
  v_media_id text;
  v_media_mime text;
  v_media_caption text;
  v_safety_classification text;
begin
  ...
  select
    ...
    contact.value->>'wa_id',
    coalesce(msg.value->'image'->>'id', msg.value->'audio'->>'id', msg.value->'voice'->>'id'),
    coalesce(msg.value->'image'->>'mime_type', msg.value->'audio'->>'mime_type', msg.value->'voice'->>'mime_type'),
    msg.value->'image'->>'caption'
  into
    v_event_field,
    v_phone_number_id,
    v_provider_event_id,
  ...
  v_message_type := coalesce(v_message_type, 'text');

  if v_message_type = 'text' then
    v_message_body := coalesce(v_message_body, '');
    v_safety_classification := null;
    ...
    if coalesce(v_media_id, '') = '' then
      return jsonb_build_object('success', false, 'code', 'MISSING_MEDIA_ID');
    end if;
    v_message_body := nullif(btrim(coalesce(v_media_caption, '')), '');
    v_safety_classification := 'whatsapp_image';
    if v_message_body is null then
      v_message_body := '[Customer sent an image]';
    end if;
  elsif v_message_type in ('audio', 'voice') then
    if coalesce(v_media_id, '') = '' then
      return jsonb_build_object('success', false, 'code', 'MISSING_MEDIA_ID');
    end if;
    v_message_body := '[Customer sent a voice note]';
    v_safety_classification := 'whatsapp_audio';
  else
    return jsonb_build_object('success', true, 'code', 'UNSUPPORTED_MESSAGE_TYPE', 'processed', false, 'messageType', v_message_type);
  end if;
