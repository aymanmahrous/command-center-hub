-- Switch WhatsApp ingress to Meta Phone Number ID linked under WABA 2575106602931255 (relax).
-- Phone Number ID: 1276699008856128 (Meta display: +971 58 821 9130).
...
AS $function$
declare
  v_expected_phone_number_id constant text := '1276699008856128';
...
begin
  ...
  if v_message_type = 'image' then
    v_media_caption := nullif(btrim(coalesce(v_media_caption, '')), '');
    v_message_body := coalesce(v_media_caption, '[Customer sent an image]');
    v_safety_classification := 'whatsapp_image';
    if v_media_id is null then
      return jsonb_build_object('success', false, 'code', 'IMAGE_MEDIA_ID_MISSING');
    end if;
  else
  ...
  return jsonb_build_object(
    ...
    'language', v_language,
    'messageType', coalesce(v_message_type, 'text'),
    'mediaId', v_media_id,
    'mediaMimeType', v_media_mime
  );
end;
$function$;
