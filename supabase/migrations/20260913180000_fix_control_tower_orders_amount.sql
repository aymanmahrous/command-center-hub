-- Fix get_staff_control_tower_summary: public.orders has no amount column.
-- Keep orderCount; return orderTotal as 0 until orders.amount exists.
...
begin
  ...
  select count(*) into v_failed_jobs from public.background_jobs where status in ('failed','dead');
