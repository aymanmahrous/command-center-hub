-- Future scheduled publishes defer owner authorization to authorize_next_due_scheduled_* (n8n/service_role).
-- Manual Request Publish and batch approve only prepare jobs; auth is minted at due time.
...
begin
  ...
  if found
     and v_content.status = 'scheduled'
     and v_content.scheduled_for is not distinct from v_publish_at
     and coalesce(v_job.next_retry_at, v_job.created_at) = v_publish_at then
    return jsonb_build_object(
      'success', true,
  ...
  if found then
    ...
    set status = 'dead',
        last_error = 'MANUAL_PUBLISH_RESCHEDULED',
        updated_at = v_now
    where id = v_job.id;
  end if;
...
-- Clear premature manual authorizations that would never survive until scheduled_for.
update public.owner_publish_authorizations opa
set revoked_at = now(),
    revoked_reason = 'PREMATURE_MANUAL_AUTH_CLEARED'
...
where opa.content_item_id = ci.id
  ...
  and opa.expires_at < ci.scheduled_for;
