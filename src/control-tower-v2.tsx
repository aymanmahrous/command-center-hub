...
export default function ControlTowerV2({ session, onSessionExpired }: { session: Session; onSessionExpired: () => void }) {
  ...
  return <div className="v2-command-home" dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    {commandOpen && <div className="v2-command-backdrop" role="presentation" onMouseDown={() => setCommandOpen(false)}><section className="v2-command-modal" role="dialog" aria-modal="true" aria-label={language === "ar" ? "البحث والأوامر" : "Search and commands"} onMouseDown={(event) => event.stopPropagation()}><div className="v2-command-input"><Command size={18} /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder={language === "ar" ? "ابحث أو اكتب أمرًا…" : "Search or type a command…"} /><kbd>ESC</kbd></div><div className="v2-command-results"><button type="button" onClick={() => go("inbox")}><MessageCircle />{language === "ar" ? "المحادثات التي تحتاج تدخلاً بشرياً" : "Conversations needing human attention"}<ArrowRight /></button><button type="button" onClick={() => go("content")}><Sparkles />{language === "ar" ? "المحتوى الذي يحتاج موافقتي" : "Content needing approval"}<ArrowRight /></button><button type="button" onClick={() => go("planner")}><CalendarDays />{language === "ar" ? "الحجوزات المعلقة" : "Pending bookings"}<ArrowRight /></button><button type="button" onClick={() => go("crm")}><Users />{language === "ar" ? "العملاء المحتملون" : "Leads"}<ArrowRight /></button></div></section></div>}
  </div>;
}
