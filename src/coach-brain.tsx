...
const INITIAL_CASE: CaseInput = { age: "", level: "", stroke: "", issue: "", observation: "", goal: "", diagnosis: "", healthNotes: "" };

const HIGH_RISK_TERMS = ["uncontrolled seizure", "active seizure", "recent surgery", "chest pain", "fainting", "loss of consciousness", "severe breathing", "acute injury", "نزيف", "إغماء", "تشنجات غير مسيطر", "عملية حديثة", "ألم صدر", "فقدان الوعي"];
const REFERRAL_TERMS = ["cerebral palsy", "down syndrome", "spina bifida", "muscular dystrophy", "neurological", "post surgery", "rehabilitation", "physiotherapy", "physio", "تأهيل", "علاج طبيعي", "شلل دماغي", "متلازمة داون", "ضمور عضلي"];
...
const copy = {
  ar: {
    title: "Coach Brain", subtitle: "مساعد السباحة التكيفية ودعم التدريب المائي", privacy: "هذه الأداة مخصصة لدعم المدرب وليست أداة تشخيص أو وصف علاج طبي.", newCase: "حالة جديدة", age: "العمر", level: "المستوى", stroke: "السباحة / المهارة", issue: "المشكلة التي لاحظتها", observation: "ماذا رأيت أثناء الحصة؟", goal: "الهدف التدريبي", diagnosis: "تشخيص معروف إن وُجد (اختياري)", healthNotes: "معلومات صحية أو قيود معروفة (اختياري)", analyze: "تحليل الحالة بأمان", train: "🟢 تدريب سباحة", adapt: "🟡 سباحة تكيفية", refer: "🟠 إحالة لمختص قبل برنامج تأهيلي", stop: "🔴 إيقاف التوصية التدريبية وطلب تقييم مختص", verify: "ما يجب التحقق منه", plan: "خطة تعليمية أولية", drills: "أفكار تمارين", safety: "السلامة والحدود", sources: "مصادر وأدلة", save: "حفظ كحالة تدريبية", research: "الأبحاث ذات الصلة", noDiagnosis: "لا نستنتج تشخيصًا من السلوك أو الأداء.", noForced: "لا يوجد إجبار على الغمر أو أي مهارة تخالف استجابة الطفل.", noEvidence: "لا توجد مصادر محددة لهذه الحالة بعد؛ استخدم بوابة السلامة ولا تعتبر المخرجات تصريحًا طبيًا.", practicalUse: "الاستخدام العملي", limitation: "حدود الدليل", openSource: "فتح المصدر",
  },
  en: {
    title: "Coach Brain", subtitle: "Adaptive swimming and aquatic-support coaching assistant", privacy: "For coaching support only. It does not diagnose conditions or prescribe medical treatment.", newCase: "New case", age: "Age", level: "Level", stroke: "Stroke / skill", issue: "Observed problem", observation: "What did you observe in the session?", goal: "Coaching goal", diagnosis: "Known diagnosis, if any (optional)", healthNotes: "Known health notes or restrictions (optional)", analyze: "Analyze safely", train: "🟢 Swimming training", adapt: "🟡 Adaptive swimming", refer: "🟠 Refer before a rehabilitation program", stop: "🔴 Stop coaching recommendation and seek professional assessment", verify: "What to verify", plan: "Initial teaching plan", drills: "Drill ideas", safety: "Safety and boundaries", sources: "Sources and evidence", save: "Save as coaching case", research: "Relevant research", noDiagnosis: "Do not infer a diagnosis from behavior or performance.", noForced: "No forced submersion or skill should be used against the swimmer's response.", noEvidence: "No specific source was selected for this case yet; use the safety gate and do not treat output as medical clearance.", practicalUse: "Practical use", limitation: "Evidence limitation", openSource: "Open source",
  },
};
...
export default function CoachBrain({ language = "ar" }: CoachBrainProps) {
  ...
  return (
    <main className="coach-brain" dir={language === "ar" ? "rtl" : "ltr"}>
        ...
        <section className="coach-brain__card coach-brain__result" aria-live="polite">
          {!analyzed ? (
            ...
            <>
              ...
              <section className="coach-brain__sources">
                <h3><BookOpen size={16} /> {t.sources}</h3>
                {evidence.length === 0 ? <p>{t.noEvidence}</p> : evidence.map((source) => <article key={source.id} className="coach-brain__evidence"><strong>{source.title}</strong><small>{source.year} · {source.evidenceLevel}</small><p><b>{t.practicalUse}:</b> {source.practicalUse}</p><p><b>{t.limitation}:</b> {source.limitation}</p><a href={source.url} target="_blank" rel="noreferrer">{t.openSource}</a></article>)}
              </section>
      ...
      <section className="coach-brain__research"><BookOpen size={18} /><div><strong>{t.research}</strong><span>{language === "ar" ? "مصادر منتقاة حاليًا؛ الربط الحي بمصادر جديدة سيكون لاحقًا بعد التحقق، وليس عبر إرسال بيانات الطفل." : "Curated sources are shown now; live evidence retrieval comes later after verification and never requires sending a child's identity."}</span></div></section>
    </main>
  );
}
