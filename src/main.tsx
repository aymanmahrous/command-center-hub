...
function PushInstallBar({ session, language }: { session: Session; language: Language }) {
  ...
  return <div className="push-install-bar">
    ...
    {installEvent && (
      <button type="button" className="text-button" onClick={() => { installEvent.prompt(); setInstallEvent(null); }}>{language === "ar" ? "تثبيت التطبيق" : "Install Command Center"}</button>
    )}
